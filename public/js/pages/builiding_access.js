...
$(document).ready(function () {
	...
	arr.push('</div>');
	arr.push('<div class="col-lg-4">');
	arr.push(generate_building_qr_code("urn:template:CSIR:csir-building-access-card-d54c3e94-867d-48fe-a9e7-32526b838c6f", '[{"building_number": "9"}]', 'Building 9'));
	arr.push('</div>');
	arr.push('<div class="col-lg-4">');
...
function generate_building_qr_code(template_id, fields_and_values_required, building_number) {
	...
	arr.push('<p class="card-text" style="text-align: center">Please scan the following QR code to verify access.</p>');
	arr.push('</div>');
	arr.push('</div>');
